...
function init(x, y) {
  ...
  for (var j = 0; j < x; j++) rr[j] = new Array(y);
  ...
  for (j = 0; j < N; j++)
    for (i = 0; i < N; i++)
      document.getElementsByTagName("TR")[i].getElementsByTagName("TD")[
        j
      ].style.backgroundImage = "none";

  document.speed_form.choice_velo[0].checked = true;

  for (i = 0; i < 92; i++) found_solutions[i] = "";
  deleteSolutions();
}
...
function get_radio(radioGroup) {
  ...
  for (i = 0; i < radioGroup.length; i++) {
    if (radioGroup[i].checked) chosen = radioGroup[i];
  }
  return chosen;
}
...
function paintBoard(n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; j < n; j++) {
      ...
      if ((i + j) % 2 != 0) oCell.className = "c2";
      else oCell.className = "c1";
...
function deleteBoard() {
  ...
  if (
    document.getElementById("oTableContainer").childNodes[0].nodeName != "TABLE"
  )
    Node = document.getElementById("oTableContainer").childNodes[1];
  else Node = document.getElementById("oTableContainer").childNodes[0];
...
function deleteSolutions() {
  ...
  var remove = document
    .getElementById("xxxxxx_left")
    .parentNode.removeChild(kn_xxxxxx_left);
  var y = document.createElement("div");
  parent.appendChild(y).id = "xxxxxx_left";

  var kn_xxxxxx_mid = document.getElementById("xxxxxx_mid");
  var remove = document
    .getElementById("xxxxxx_mid")
    .parentNode.removeChild(kn_xxxxxx_mid);
  var y = document.createElement("div");
  parent.appendChild(y).id = "xxxxxx_mid";

  var kn_xxxxxx_right = document.getElementById("xxxxxx_right");
  var remove = document
    .getElementById("xxxxxx_right")
    .parentNode.removeChild(kn_xxxxxx_right);
  var y = document.createElement("div");
...
function free_f(px, py) {
  ...
  for (x = 0; x < py; x++) {
    if (los[x] == px || Math.abs(los[x] - px) == Math.abs(x - py)) return 0;
  }
  return 1;
}
    ...
    if (cell_del != 0) {
      ...
      if (old[i] != 0) {
        ...
        document
          ...
          [cell_row].getElementsByTagName("TD")[
          cell_col
        ].style.backgroundImage = "none";
      }
    }
...
function click_queenPosition(event) {
  ...
  if (los[e2] == e1 + 1) {
    document.getElementsByTagName("TR")[e1].getElementsByTagName("TD")[
      e2
    ].style.backgroundImage = "none";
    los[e2] = 0;
    click_count--;
  ...
  if (p_flag == 1) {
    ...
    document.getElementsByTagName("TR")[e1].getElementsByTagName("TD")[
      e2
    ].style.backgroundImage = "url(img/ChessQueen2.png)";
    if (click_count == N) {
      alreadyFound();
    }
  } else alert("Pozycja niepoprawna");
}
...
function printSolution() {
  ...
  if (mode_flag == 0) {
    ...
    s = s + "]" + "<br/>";
    if (print_count < 30) document.getElementById("xxxxxx_left").innerHTML += s;
...
function alreadyFound() {
  ...
  for (var j = 0; j < 92; j++) {
    if (found_solutions[j] == tmp) f_flag = 1;
  }
  if (f_flag == 1) {
    ...
    alert("Position found " + " (" + f_s_count + " z " + all + ")");
    printSolution();
  }
}
...
function queenPosition() {
  ...
  if (x > 0) {
    var wiersz = x;
    var kolumna = cc;
    document.getElementsByTagName("TR")[wiersz - 1].getElementsByTagName("TD")[
      kolumna
    ].style.backgroundImage = "url(img/ChessQueen2.png)";
  }
